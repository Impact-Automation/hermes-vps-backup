...
When wiring MiniMax into Hermes on this VPS:

- Hermes already supports provider `minimax` and model `MiniMax-M2.7`.
- Verify Hermes directly with:

```bash
hermes chat -q "Reply with exactly OK" --provider minimax -m MiniMax-M2.7 -Q
```

- The current Hermes codebase/catalog does **not** expose a separate `MiniMax-M2.7-highspeed` model ID.
- If a user asks for "M2.7 high speed", the practical working setup is usually:
  - authenticate `mmx`
  - set the `MINIMAX_API_KEY` environment variable for Hermes
  - use Hermes with `MiniMax-M2.7`
- Do **not** promise a Hermes `highspeed` model option unless the local codebase/catalog has been updated to include it.

Minimal setup flow used successfully here:
...
# verify CLI
mmx text chat --message "user:Reply with exactly OK" --output json --quiet
